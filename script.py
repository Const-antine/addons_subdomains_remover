...
if e:
	...
	if ans == "Y":
		...
		if bul == "Y":
			for i in sdoms:
				#b = subprocess.Popen(['cpapi2', '--user=c0nst', 'SubDomain', 'delsubdomain, 'domain=i'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
				b = subprocess.call(['cpapi2', '--user=c0nst', 'SubDomain', 'delsubdomain', 'domain=%s' % (i)])
				print("Removed")
		else:
			print('Break!') 
else:
